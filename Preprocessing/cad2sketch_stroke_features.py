...
def point_on_line_extension(p, a, b, tol=1e-3):
    """
    Check if point p lies on the infinite 3D line defined by points a and b.
    Uses perpendicular distance to the line for numerical robustness.
    """
    ...
    if ab_norm < 1e-12:
        return np.linalg.norm(ap) < tol  # a == b, treat as single point

    cross = np.cross(ab, ap)
    distance = np.linalg.norm(cross) / ab_norm  # Perpendicular distance to the line
    return distance < tol
