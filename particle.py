...
class Particle():
    ...
    def mark_off_new_strokes(self, stroke_to_brep, stroke_to_edge_circle):
        ...
        if self.used_strokes == -1:
            ...
            self.used_circle_strokes = new_circle_mark_off
            return True
        
        # case 1: added a circle
        if new_circle_mark_off > self.used_circle_strokes and new_strokes_mark_off > self.used_strokes:
            return True
        if new_circle_mark_off > self.used_circle_strokes and new_strokes_mark_off <= self.used_strokes:
            return False

        # case 2: added a non-circle
        if self.used_strokes + 4 < new_strokes_mark_off:
            self.used_strokes = new_strokes_mark_off
            self.used_circle_strokes = new_circle_mark_off
            return True
...
def predict_sketch(gnn_graph, data_idx):
...
def do_sketch(gnn_graph, data_idx):
    selected_loop_idx, sketch_selection_mask, idx_prob= predict_sketch(gnn_graph, data_idx)
...
def predict_fillet(gnn_graph, data_idx):
