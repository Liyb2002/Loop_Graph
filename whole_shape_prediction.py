...
def eval():
    load_models()
    # Load the dataset
    dataset = Preprocessing.dataloader.Program_Graph_Dataset('dataset/simple_eval')
    print(f"Total number of shape data: {len(dataset)}")
    ...
    with torch.no_grad():
        for gnn_graph, loop_selection_mask in tqdm(zip(val_graphs, val_masks), desc=f"Evaluation"):
            x_dict = graph_encoder(gnn_graph.x_dict, gnn_graph.edge_index_dict)
            output = graph_decoder(x_dict)
            ...
            if output_selected_idx == mask_selected_idx:
                ...
                Encoders.helper.vis_whole_graph(gnn_graph, output)
